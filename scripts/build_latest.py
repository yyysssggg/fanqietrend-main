...
**🔥 题材信号**
用1-2句话总结当前分类的主流题材和高频元素，点明哪些设定扎堆出现。不要预设男频或女频，只根据书名、简介和榜单数据判断。

**📖 读者偏好**
用1句话概括读者口味方向（爽感、升级、情绪、日常、悬疑、甜虐等），以及金手指或核心卖点偏好。

**📈 榜单变化**
结合新上榜、掉榜、排名上升和阅读增长，判断这个分类当前是稳定、换血、上冲还是回落。

**🆕 新上榜作品**
列出值得注意的新上榜书名，每本用一句话点评题材亮点或差异化卖点；没有新上榜就写“暂无明显新增”。

**💡 值得关注**
挑1-2本有差异化潜力或增长信号的作品，说明理由，并给出一句编辑判断。

要求：每个板块1-2句话，总字数220字以内。语言简洁专业，像行业快报，避免空泛套话。"""
...
def build_batch_ai_prompt(batch: list) -> str:
    ...
    return (
        ...
        f"为番茄小说的多个分类榜单分别生成结构化分析。"
        f"不要预设男频或女频，只根据书名、简介和榜单数据判断。\n\n"
        f"{all_sections}\n\n"
        f"## 输出要求\n\n"
        f"请严格按照以下格式，为每个分类分别输出分析。"
        f"每个分类的分析必须包裹在对应的标记中：\n\n"
        f"{output_examples}\n\n"
        f"每个板块1-2句话，每个分类总字数220字以内。"
        f"语言简洁专业，像行业快报，避免空泛套话。\n"
