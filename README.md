...
> 📚 面向**番茄小说榜单追踪**，可配置为男频、女频或其他番茄榜单；每日自动追踪排行数据并结合 AI 生成趋势分析，部署为精美的在线看板。
...
| 功能 | 说明 |
|------|------|
| 🕷️ 自动爬取 | 每日定时抓取配置榜单下各个分类的 Top 30，可切换男频、女频或其他榜单 |
...
如果想切换追踪范围，可以在仓库 **Settings** → **Secrets and variables** → **Actions** → **Variables** 中添加这些变量：
...
切换榜单时建议同时修改 `FANQIE_SNAPSHOT_PREFIX`，避免不同榜单的数据混在一起。已有旧数据仍会保留，不影响新榜单运行。
